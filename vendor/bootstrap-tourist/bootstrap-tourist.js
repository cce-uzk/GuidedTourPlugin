	...
	Tour = (function () {
		...
		function Tour(options) {
			...
			if (typeof (this._options.sanitizeFunction) == "function") {
				...
				var whiteListAdditions = {
					"*": ["disabled"], // fix: important for disabling next and prev button (on first an last tour step)
		...
		Tour.prototype.getStep = function (i) {
			if (this._options.steps[i] != null) {
				...
				// Check if the element attribute is a function definition
				if (typeof(this._options.steps[i].element) === "string" && this._options.steps[i].element.startsWith('func:')) {
					// It's a function: remove prefix and execute
					const functionBody = this._options.steps[i].element.slice(5); // Remove 'func:' prefix
					try {
						let result = eval(functionBody);
						if (result) {
							this._options.steps[i].element = result;
						} else {
							this._options.steps[i].element = null;
							this._options.steps[i].orphan = true;
							console.log("Error binding step to element.")
						}
					} catch (error) {
				...
				// Process event attributes for function definitions in string format and converts them to actual executable functions
				const eventAttributes = [
					'onNext', 'onPrev', 'onShow', 'onShown', 'onHide', 'onHidden',
					'onPause', 'onResume', 'onRedirectError'
				];

				eventAttributes.forEach(attr => {
					const attrValue = this._options.steps[i][attr];
					if (typeof attrValue === 'string' && attrValue.length > 0) {
						...
						// Log or handle cases where the attribute is not a proper string
						console.info(`Attribute ${attr} is not a valid function string or is empty.`);
					}
				});
		...
		Tour.prototype._template = function (step, i) {
			...
			if (step.prev < 0) {
				...
				$prev.prop('tabindex', -1);
			}
			if (step.next < 0) {
				$next.addClass('disabled');
				$next.attr('disabled', 'disabled');
				//$next.prop('disabled', true);
				$next.prop('tabindex', -1);
			}
		...
		// Reinstate the code that moves a non-orphan popover when window is scrolled
